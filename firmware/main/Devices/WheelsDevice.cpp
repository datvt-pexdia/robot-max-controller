...
// ==========================================================
#ifndef WHEELS_DEBUG
#define WHEELS_DEBUG 1           // 0: quiet, 1: verbose logs
#endif

#ifndef COMM_PERIOD_MS
#define COMM_PERIOD_MS 1         // cadence for keep-alive motor command (DriveTest ~1ms)
#endif
...
// Direction bytes
// Note: on MAX, "robot forward" means: Left = anticlockwise (0x34), Right = clockwise (0x24)
...
static inline uint8_t mapSpeedPercentToByte(int pctAbs /*0..100*/) {
  ...
  // Arduino map returns long; min/max inclusive behavior is OK for this range
...
// ----------------------------------------------------------
inline void WheelsDevice::sendRL_(uint8_t rightByte, uint8_t leftByte) {
...
void WheelsDevice::tick(uint32_t now) {
  ...
  if (motorInitialized && channelMOTOR && (now - lastCommandMs) >= COMM_PERIOD_MS) {
    // keep-alive direction bytes to maintain motion
    sendRL_(current.rightCmd, current.leftCmd);
    lastCommandMs = now;
  }
#endif
